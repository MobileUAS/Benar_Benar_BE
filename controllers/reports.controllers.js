...
module.exports = {
    ...
    createReport: async (req, res) => {
        try {
            const { jumlah, stokMasuk, stokKeluar, barangID, suplierID, distributorID } = req.body;
            const newReport = new Reports({ jumlah, stokMasuk, stokKeluar, barangID, suplierID, distributorID });
    ...
    updateReport: async (req, res) => {
        try {
            ...
            const updatedReport = await Reports.findByIdAndUpdate(
                req.params.id,
                { jumlah, stokMasuk, stokKeluar, barangID, suplierID, distributorID },
                { new: true }
            ).populate('barangID', 'namaproduct')
             .populate('suplierID', 'nama')
             .populate('distributorID', 'nama');
